...
import {
  useGetLearnerImportTemplate,
  useUploadLearnerImport,
  useGetLearnerImportJob,
  useListLearnerImportJobRows,
  useResolveLearnerImportJobRow,
  useConfirmLearnerImportJob,
  useCancelLearnerImportJob,
  useDownloadLearnerImportErrors,
  getGetLearnerImportTemplateQueryKey,
  getGetLearnerImportJobQueryKey,
  getListLearnerImportJobRowsQueryKey,
  getDownloadLearnerImportErrorsQueryKey,
  type LearnerImportJobRow,
  type LearnerImportRowClassification,
...
const CLASSIFICATION_META: Record<
  LearnerImportRowClassification,
...
> = {
  ...
  possible_duplicate: { label: "Possible duplicate", className: "bg-orange-100 text-orange-800 border-orange-200 dark:bg-orange-900/30 dark:text-orange-400" },
...
export default function LearnerImportPage() {
  ...
  const fileInputId = "learner-import-file-input";
  const pageSize = 25;

  // ---------------------------------------------------------------------
  // Template download
  // ---------------------------------------------------------------------
  const templateQuery = useGetLearnerImportTemplate({
    query: { enabled: false, queryKey: getGetLearnerImportTemplateQueryKey() },
  });
  const handleDownloadTemplate = async () => {
    const result = await templateQuery.refetch();
    if (result.data) downloadCsv(result.data.csv, result.data.filename ?? "learner-import-template.csv");
  ...
  // Upload
  // ---------------------------------------------------------------------
  const uploadMutation = useUploadLearnerImport();
  ...
  // ---------------------------------------------------------------------
  const jobQuery = useGetLearnerImportJob(jobId ?? 0, {
    query: {
      queryKey: getGetLearnerImportJobQueryKey(jobId ?? 0),
      enabled: jobId != null,
  ...
  const rowsParams = {
    page,
    pageSize,
    classification: classificationFilter !== allValue ? (classificationFilter as LearnerImportRowClassification) : undefined,
  };
  const rowsQuery = useListLearnerImportJobRows(jobId ?? 0, rowsParams, {
    query: {
      queryKey: getListLearnerImportJobRowsQueryKey(jobId ?? 0, rowsParams),
      enabled: jobId != null && job?.status === "ready",
    },
  });

  const resolveMutation = useResolveLearnerImportJobRow();
  ...
  // Confirm
  // ---------------------------------------------------------------------
  const confirmMutation = useConfirmLearnerImportJob();
  const handleConfirm = () => {
    if (!jobId) return;
  ...
  // Cancel / start over
  // ---------------------------------------------------------------------
  const cancelMutation = useCancelLearnerImportJob();
  ...
  // Error report
  // ---------------------------------------------------------------------
  const errorsQuery = useDownloadLearnerImportErrors(jobId ?? 0, {
    query: { enabled: false, queryKey: getDownloadLearnerImportErrorsQueryKey(jobId ?? 0) },
  });
  const handleDownloadErrors = async () => {
    const result = await errorsQuery.refetch();
    if (result.data) downloadCsv(result.data.csv, result.data.filename ?? `learner-import-${jobId}-errors.csv`);
  ...
  return (
    <div className="p-6 md:p-8 max-w-6xl mx-auto w-full">
      <Breadcrumbs items={[{ label: "Learners", href: "/learners" }, { label: "Import from CSV" }]} />
      ...
      <div className="flex items-center gap-4 mb-8 page-transition-enter">
        ...
        <div>
          <h1 className="text-3xl font-bold tracking-tight text-foreground">Import Learners</h1>
          <p className="text-muted-foreground mt-1">Bulk upload apprentices from a CSV file.</p>
        </div>
      </div>
                      ...
                      rows.map((row) => (
                        <TableRow key={row.id}>
                          ...
                          <TableCell className="text-sm">
                            ...
                            <div className="text-xs text-muted-foreground font-mono">{row.rawData.learner_reference}</div>
                          </TableCell>
                            ...
                            {row.rawData.cohort_name ? (
                              row.cohortMatchStatus === "matched" ? (
                                ...
                                <span className="text-amber-600 dark:text-amber-400 text-xs">
                                  {row.rawData.cohort_name} ({row.cohortMatchStatus?.replace("_", " ")})
                                </span>
                              )
                            ) : "-"}
                          </TableCell>
                          <TableCell className="text-xs text-rose-600 dark:text-rose-400 max-w-[220px]">
        ...
        <Card className="shadow-sm max-w-2xl page-transition-enter">
          ...
          <CardContent className="pt-6 space-y-6">
            ...
            <div className="flex gap-2">
              ...
              <Button className="flex-1" onClick={() => setLocation("/learners")}>Back to Learners</Button>
            </div>
          </CardContent>
        </Card>
      )}
      ...
      {job && status === "cancelled" && (
        <Card className="shadow-sm max-w-2xl">
          <CardContent className="flex flex-col items-center justify-center py-16 text-center">
            ...
            <p className="text-sm text-muted-foreground max-w-sm mb-4">No learners were created or changed.</p>
            <Button onClick={handleStartOver}>Start New Import</Button>
          </CardContent>
        </Card>
      )}
