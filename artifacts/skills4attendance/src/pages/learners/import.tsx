...
import { useGetLearnerCsvTemplate, usePreviewLearnerCsv, useImportLearnerCsv, CsvPreviewResult, getGetLearnerCsvTemplateQueryKey } from "@workspace/api-client-react";
...
import { Download, Upload, Loader2, AlertCircle, FileText, CheckCircle2, ArrowLeft, Check, AlertTriangle } from "lucide-react";
...
export default function LearnerImportPage() {
  ...
  const getTemplateMutation = useGetLearnerCsvTemplate({
    query: { enabled: false, queryKey: getGetLearnerCsvTemplateQueryKey() } // we trigger it manually
  });
  const previewMutation = usePreviewLearnerCsv();
  const importMutation = useImportLearnerCsv();
  ...
  const handleDownloadTemplate = async () => {
    try {
      ...
      if (result.data) {
        ...
        a.href = url;
        a.download = result.data.filename || 'learners_template.csv';
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files[0]) {
      ...
      setPreviewResult(null); // clear old preview
    }
  };
    ...
    importMutation.mutate({ data: formData as any }, {
      onSuccess: (data) => {
        toast({
          ...
          description: `Successfully imported ${data.imported} learners. Skipped ${data.skipped}.`
        });
