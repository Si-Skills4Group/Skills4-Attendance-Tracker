...
import { Loader2, Download, CalendarX, ChevronLeft, ChevronRight } from "lucide-react";
...
export default function AbsenceReportPage() {
  ...
  const [absenceType, setAbsenceType] = React.useState<AbsenceType>("unauthorised");
  ...
  const { data, isLoading, isError } = useGetAbsenceReport(queryParams);

  const handleExport = async () => {
    setIsExporting(true);
    try {
      const csv = await exportAbsenceReport({ ...queryParams });
      downloadCsv(csv, `absence-${absenceType}-report.csv`);
    } catch (err: any) {
  ...
  return (
    <div className="p-6 md:p-8 max-w-7xl mx-auto w-full">
      <Breadcrumbs items={[{ label: "Reports", href: "/reports" }, { label: "Absence Analysis" }]} />
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6 page-transition-enter">
        <div>
          <h1 className="text-3xl font-bold tracking-tight text-foreground flex items-center gap-2">
            <CalendarX className="w-7 h-7 text-primary" /> Absence Analysis
          </h1>
          <p className="text-muted-foreground mt-1">Authorised and unauthorised absences, session by session.</p>
        </div>
      ...
      <Tabs value={absenceType} onValueChange={(v) => { setAbsenceType(v as AbsenceType); setPage(1); }} className="mb-4 page-transition-enter stagger-1">
        <TabsList>
          <TabsTrigger value="unauthorised">Unauthorised</TabsTrigger>
          <TabsTrigger value="authorised">Authorised</TabsTrigger>
        </TabsList>
      </Tabs>
      ...
      {isLoading ? (
        ...
        <Card className="shadow-sm"><CardContent className="p-8 text-center text-muted-foreground">Could not load the absence report.</CardContent></Card>
      ) : (
        <Card className="shadow-sm overflow-hidden">
          <div className="overflow-x-auto">
            <Table>
              ...
              <TableBody>
                {data?.items.length === 0 ? (
                  <TableRow><TableCell colSpan={6} className="text-center p-8 text-muted-foreground">No {absenceType} absences found for these filters.</TableCell></TableRow>
                ) : (
                  data?.items.map((row, i) => (
                    <TableRow key={`${row.sessionId}-${row.learnerId}-${i}`}>
                      ...
                      <TableCell className="text-right font-mono">{row.plannedDurationHours}</TableCell>
                    </TableRow>
                  ))
                )}
              </TableBody>
